...
const adminPath = path.join(rootDir, "data", "china-admin-blocks-280-317.json");
const controlPath = path.join(rootDir, "data", "china-block-control-timeline-280-317.json");

const geometryDatasetId = "china-admin-block-map-280-317";
const controlDatasetId = "china-block-control-timeline-280-317";
...
function assertIntegrity({ adminDataset, controlDataset }) {
  const blockIds = new Set();
  const controllerIds = new Set((controlDataset.controllers ?? []).map((controller) => controller.id));
  const errors = [];

  for (const block of adminDataset.blocks ?? []) {
    ...
    if (block.parent && !blockIds.has(block.parent)) {
      // Parent order is not guaranteed, checked again after all ids are collected below.
    }
  }

  for (const block of adminDataset.blocks ?? []) {
    if (block.parent && !blockIds.has(block.parent)) {
      errors.push(`Unresolved parent for ${block.id}: ${block.parent}`);
    }
    if (block.controlBlockId && !blockIds.has(block.controlBlockId)) {
  ...
  if (errors.length > 0) {
    throw new Error(`China 280-317 map geometry import blocked:\n${errors.slice(0, 30).join("\n")}`);
  }
}

const [adminDataset, controlDataset] = await Promise.all([
  loadJson(adminPath),
  loadJson(controlPath),
]);

assertIntegrity({ adminDataset, controlDataset });
...
try {
  ...
  const insertGeometryDataset = db.prepare(`
    INSERT INTO map_geometry_datasets (
      id, schema_version, model, region_id, period_id, civilization_id, label,
      time_start, time_end, coordinate_system, source_note, review_status, raw_json
    )
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const insertFeature = db.prepare(`
    INSERT INTO map_features (
      id, dataset_id, name, name_zh, feature_type, admin_level, parent_feature_id,
      control_feature_id, center_lon, center_lat, label_lon, label_lat, min_lon, min_lat,
      max_lon, max_lat, area_hint, confidence, approximate, review_status, notes, raw_json
    )
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  runInsert(insertGeometryDataset, [
    ...
    null,
    null,
    "China Western Jin commandery-style geometry 280-317",
    adminDataset.range?.[0] ?? 280,
    adminDataset.range?.[1] ?? 317,
    adminDataset.coordinateSystem ?? "wgs84-lonlat",
    adminDataset.notes ?? null,
    "derived-review-needed",
    JSON.stringify({
      schemaVersion: adminDataset.schemaVersion,
      model: adminDataset.model,
      range: adminDataset.range,
      basisYear: adminDataset.basisYear,
      boundaryBasisYear: adminDataset.boundaryBasisYear,
  ...
  for (const block of adminDataset.blocks ?? []) {
    ...
    runInsert(insertFeature, [
      ...
      "derived-review-needed",
      Array.isArray(block.notes) ? block.notes.join(" ") : block.notes ?? null,
  ...
  runInsert(insertControlDataset, [
    ...
    "china",
    null,
    "China Western Jin control timeline 280-317",
    controlDataset.range?.[0] ?? 280,
    controlDataset.range?.[1] ?? 317,
  ...
  console.log(`Imported ${adminDataset.blocks?.length ?? 0} China 280-317 map features.`);
