...
本文档定义中国区域页第一版“三国区块地图”模型。目标是接近策略游戏里的区块地图，而不是精确 GIS 边界图。第一版区块形状允许简化、允许近似，重点先把“区块 + 控制权时间线 + 状态不确定性”跑通。

## Block Layer

`block layer` 是中国区域页的主要交互层。

- 州级区块：冀州、幽州、并州、青州、兖州、豫州、徐州、司隶、雍州、凉州、荆州、扬州、益州、交州等。
- 重点郡/国区块：第一版先单独拆出汉中、辽东这类需要独立变色的区域。
...
`control timeline` 记录每个区块在不同年份由哪个势力控制。UI 根据当前时间线年份查找生效记录，并用控制者颜色给区块上色。
...
控制者颜色写在 `data/china-control-timeline-190-280.json` 的 `controllers` 中，保证数据和 UI 同步。
